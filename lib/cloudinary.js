...
export const getSurveyPdfFolder = (siteName, surveyRef) => {
  // Clean values to avoid issues with special characters - USING SAME METHOD AS IMAGES
  const cleanSiteName = siteName ? siteName.replace(/[:/\\?*"|<>]/g, '-') : 'unknown-site';
  const cleanRef = surveyRef ? surveyRef.replace(/[:/\\?*"|<>]/g, '-') : 'unknown';
  
  // This matches the image path pattern but without a category subfolder
...
export const getCloudinaryPdfUrl = (publicId) => {
  if (!publicId) {
    console.warn("getCloudinaryPdfUrl called with null/empty publicId");
    return null;
  }
  
  const cloudName = process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || "dnu5hunya";
  
  // Ensure publicId ends with .pdf, but avoid adding it twice
  const formattedPublicId = publicId.endsWith('.pdf') ? publicId : `${publicId}.pdf`;
  
  const url = `https://res.cloudinary.com/${cloudName}/raw/upload/${formattedPublicId}`;
  
  // Add debug logging in development
  if (process.env.NODE_ENV !== 'production') {
    console.log(`PDF URL generated: ${url} (from publicId: ${publicId})`);
  }
