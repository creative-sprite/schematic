...
export async function POST(request) {
  ...
  try {
    ...
    if (group && group.groupName) {
      // Process groupPhoneNumbers and ensure primary flag
      if (group.groupPhoneNumbers) {
        if (group.groupPhoneNumbers.length > 0 && !group.groupPhoneNumbers.some(p => p.isPrimary)) {
          group.groupPhoneNumbers[0].isPrimary = true;
        }
      }
      
      // Process groupEmails and ensure primary flag
      if (group.groupEmails) {
        if (group.groupEmails.length > 0 && !group.groupEmails.some(e => e.isPrimary)) {
          group.groupEmails[0].isPrimary = true;
        }
      }
    ...
    if (chain && chain.chainName) {
      ...
      // Process chainPhoneNumbers and ensure primary flag
      if (chainData.chainPhoneNumbers) {
        if (chainData.chainPhoneNumbers.length > 0 && !chainData.chainPhoneNumbers.some(p => p.isPrimary)) {
          chainData.chainPhoneNumbers[0].isPrimary = true;
        }
      }
      
      // Process chainEmails and ensure primary flag
      if (chainData.chainEmails) {
        if (chainData.chainEmails.length > 0 && !chainData.chainEmails.some(e => e.isPrimary)) {
          chainData.chainEmails[0].isPrimary = true;
        }
      }
    ...
    if (site && site.siteName) {
      ...
      // Process sitePhoneNumbers and ensure primary flag
      if (siteData.sitePhoneNumbers) {
        if (siteData.sitePhoneNumbers.length > 0 && !siteData.sitePhoneNumbers.some(p => p.isPrimary)) {
          siteData.sitePhoneNumbers[0].isPrimary = true;
        }
      }
      
      // Process siteEmails and ensure primary flag
      if (siteData.siteEmails) {
        if (siteData.siteEmails.length > 0 && !siteData.siteEmails.some(e => e.isPrimary)) {
          siteData.siteEmails[0].isPrimary = true;
        }
      }
    ...
    if (contact && contact.contactFirstName && contact.contactLastName) {
      ...
      // Process contactPhoneNumbers and ensure primary flag
      if (contactData.contactPhoneNumbers) {
        if (contactData.contactPhoneNumbers.length > 0 && !contactData.contactPhoneNumbers.some(p => p.isPrimary)) {
          contactData.contactPhoneNumbers[0].isPrimary = true;
        }
      }
      
      // Process contactEmails and ensure primary flag
      if (contactData.contactEmails) {
        if (contactData.contactEmails.length > 0 && !contactData.contactEmails.some(e => e.isPrimary)) {
          contactData.contactEmails[0].isPrimary = true;
        }
      }
      ...
      if (contactData.contactEmails && typeof contactData.contactEmails === "string") {
        contactData.contactEmails = [{ 
          email: contactData.contactEmails, 
          location: 'Office', 
          isPrimary: true 
        }];
      }
    ...
    if (supplier && supplier.supplierName) {
      // Process supplierPhoneNumbers and ensure primary flag
      if (supplier.supplierPhoneNumbers) {
        if (supplier.supplierPhoneNumbers.length > 0 && !supplier.supplierPhoneNumbers.some(p => p.isPrimary)) {
          supplier.supplierPhoneNumbers[0].isPrimary = true;
        }
      }
      
      // Process supplierEmails and ensure primary flag
      if (supplier.supplierEmails) {
        if (supplier.supplierEmails.length > 0 && !supplier.supplierEmails.some(e => e.isPrimary)) {
          supplier.supplierEmails[0].isPrimary = true;
        }
      }
